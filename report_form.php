                    ...
                    <div class="form-group"><label class="control-label">Test </label><select required class="form-control" name="testname">
                            ...
                            if (!$conn)
                            {
                               die("Connection failed: " . mysqli_connect_error());
                                ...
                                $sql = "SELECT TestName, TestID FROM TestMaterials GROUP BY TestName ";
                                $result = mysqli_query($conn, $sql);
                                $resultCheck = mysqli_num_rows($result);
                                $testnamesArray = array();
                                $testIDArray = array();
                                //echo "<option value='test1'>". $resultCheck; 
                                $x=0;
                                if (mysqli_num_rows($result) > 0)
                                   {
                                       while($row = mysqli_fetch_assoc($result))
                                       {
                                          $testnamesArray[$x] = $row["TestName"];
                                   ...
                                   //echo "</option>";
                                $counter = 0;
                                while($counter < $resultCheck)
                                {
                                    echo "<option value=\"".$testIDArray[$counter] ."\">". $testnamesArray[$counter]."</option>";
                                    $counter++;
                                }
                            ?>
                        </select></div>
